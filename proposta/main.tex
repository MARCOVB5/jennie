...
\vspace{1em}  
\begin{center}  
    \Large\textbf{Sistema Jennie: Servidor NAS de Baixo Consumo e  Alta Confiabilidade} \\
\end{center} 
\vspace{1em}  
...
Diante desse cenário de problemas em cascata, este projeto propõe o desenvolvimento de um servidor NAS eficiente, transparente e acessível, fundamentado em uma placa \textit{Raspberry Pi 5} \cite{upton2014}. A arquitetura funcionará sob um sistema operacional customizado de código livre e aberto, garantindo total transparência e otimização de recursos. Adicionalmente, o projeto engloba a construção de módulos eletrônicos dedicados, incluindo um sistema de alimentação ininterrupta (no-break) e controle de refrigeração ativa, assegurando a estabilidade e a integridade do hardware.

\newpage

 %--- SEÇÃO: DESCRIÇÃO GERAL ---  
\section{Descrição Geral}  
O projeto consiste em um sistema de armazenamento estruturado em três componentes principais: hardware de processamento (Raspberry Pi 5), módulos eletrônicos de suporte e um ecossistema de software customizado. A Raspberry Pi atua como o núcleo do sistema, interligando o arranjo de discos rígidos através de barramentos de alta velocidade (PCIe\footnote{\textit{Peripheral Component Interconnect Express}: barramento serial de alta velocidade para conexão de periféricos internos.}). Para viabilizar a operação 24/7 de maneira confiável, um circuito \textit{no-break} incorpora o gerenciamento de baterias e circuitos lógicos para acionamento das ventoinhas conforme a demanda térmica do processador e dos discos. \\
...
\item \textbf{Orquestração de Microserviços:} Capacidade de hospedar e gerenciar aplicações isoladas (contêineres), como automações residenciais, bots de monitoramento e websites.
...
\begin{figure}[H]  
    ...
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{images/diagrama.jpg}}
...
\begin{itemize}
    \item \textbf{RF01 - Gerenciamento de Armazenamento:} O sistema deve ser capaz de reconhecer, formatar e gerenciar um arranjo de até 4 discos rígidos (HDDs) de 2.5". A escolha exclusiva por HDDs justifica-se pelo foco do projeto em alta densidade de armazenamento e soberania de dados com baixo custo de aquisição \cite{berl2010}. Visto que o gargalo de desempenho para o uso doméstico reside na interface de rede e no barramento PCIe-to-SATA\footnote{\textit{Serial ATA}: interface padrão de comunicação entre placa-mãe e dispositivos de armazenamento.} compartilhado \cite{halfacree2023}, o uso de SSDs elevaria o custo total sem proporcionar ganho perceptível em aplicações de \textit{cold storage} e servidores de mídia.
    ...
    \item \textbf{RF05 - Hospedagem:} O sistema deve permitir a execução de serviços em ambientes isolados (contêineres), garantindo que falhas em uma aplicação não afetem o sistema operacional host.
    \item \textbf{RF06 - Monitoramento e Dashboard:} O sistema deve disponibilizar uma interface web para visualização em tempo real das métricas de saúde (temperatura, tensão) e performance (uso de CPU, RAM e discos), além de permitir a configuração de alertas proativos em casos de anomalia.
...
\begin{itemize}
    \item \textbf{RNF01 - Desempenho e Eficiência:} O sistema operacional deve ser enxuto e otimizado para a arquitetura ARM\footnote{\textit{Advanced RISC Machine}: arquitetura de processadores de baixo consumo, amplamente usada em dispositivos embarcados e móveis.} embarcada, com baixo consumo de recursos e priorização da alocação de memória para aplicações do usuário.
    \item \textbf{RNF02 - Privacidade e Segurança Lógica:} A comunicação externa e a autenticação devem utilizar protocolos criptografados (ex: TLS/SSL\footnote{\textit{Transport Layer Security / Secure Sockets Layer}: protocolos de criptografia para comunicação segura em rede.}, SSH \cite{rfc4251}), garantindo que os dados em trânsito e as aplicações de servidor não sejam interceptados.
    \item \textbf{RNF03 - Confiabilidade de Dados:} O gerenciamento de armazenamento deve utilizar um sistema de arquivos moderno (ex: Btrfs ou ext4 com redundância) com suporte à verificação de integridade e prevenção contra corrupção silenciosa de dados (\textit{bit rot}) \cite{rodeh2013}.
    \item \textbf{RNF04 - Autonomia e Tolerância a Falhas:} O módulo de energia (no-break) deve manter o NAS operacional por 5 minutos após uma interrupção da rede elétrica, tempo suficiente para a conclusão do processo de shutdown seguro, prevenindo perda de blocos em disco.
    \item \textbf{RNF05 - Estrutura e Concepção Mecânica:} O sistema deverá dispor de uma estrutura mecânica para conter o Raspberry PI 5 e seus demais módulos, tais como refrigeração, circuito \textit{no-break}, entre outros.
    \item
    \textbf{RNF06 - Escalabilidade Térmica:} O sistema de arrefecimento deve ser capaz de manter a temperatura abaixo do limite de \textit{thermal throttling} mesmo sob carga de 100\% de CPU e I/O de disco por 30 minutos.
    \item \textbf{RNF07 - Arquitetura de Micro-Backend} A camada de gerenciamento deve utilizar um micro-backend de baixo \textit{overhead} (ex: Go ou Python), garantindo tempos de resposta inferiores a 200ms para requisições de status do sistema.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Sistemas Operacionais:} Construção e compilação de uma distribuição Linux customizada, configuração e compilação do \textit{kernel} e gerenciamento de processos e sistemas de arquivos \cite{tanenbaum2015}.
    ...
    \item \textbf{Sistemas Embarcados \& Eletrônica Geral 1:} Interfaceamento da Raspberry Pi com o hardware customizado, condicionamento de sinais e dimensionamento de fontes chaveadas e circuitos de carga/descarga de baterias para o no-break.
    \item \textbf{Circuitos Digitais:} Controle PWM\footnote{\textit{Pulse Width Modulation} (Modulação por Largura de Pulso): controle de potência por variação da duração de pulsos digitais.} para refrigeração e lógica digital para os sensores do hardware de suporte.
...
\begin{itemize}  
    \item \textbf{Sistema Operacional:} Utilização da distribuição Gentoo Linux como base, mas customizada para ser desprovida de pacotes não essenciais, com \textit{kernel} focado em inicialização rápida e baixo \textit{overhead}.
    \item \textbf{I/O e Rede:} \textit{Daemons} responsáveis por expor o armazenamento, como o servidores diversos e protocolos FTP/SFTP.
    \item \textbf{Abstração de Serviços (Virtualização):} Utilização de um motor de contêineres (como Docker ou Podman) para isolar e permitir hospedagem de aplicações adicionais instaladas pelos usuários (ex: servidores de mídia, \textit{pi-hole}, \textit{home assistant}).
    \item \textbf{Inteligência Monitoramento:} Rotinas de software (\textit{scripts} em \textit{bash} ou \textit{Python}) que recebem interrupções do circuito de no-break via pinos GPIO\footnote{\textit{General Purpose Input/Output}: pinos de entrada/saída de propósito geral do Raspberry Pi.} e executam o desligamento seguro, além de monitorar e ajustar a velocidade das ventoinhas \cite{donovan2015}.
\end{itemize}  
...
\begin{itemize}  
    ...
    \item \textbf{Controladora de Armazenamento:} Interfaceamento via \textit{Hat}\footnote{\textit{Hardware Attached on Top}: placa de expansão padronizada para o Raspberry Pi.} PCIe-to-SATA, garantindo largura de banda suficiente para arranjos RAID\footnote{\textit{Redundant Array of Independent Disks}: combinação de múltiplos discos em uma unidade lógica para desempenho e/ou redundância.} e I/O intensivo \cite{halfacree2023}.
    \item \textbf{Módulo de No-Break:} Circuito customizado composto por células de Lítio (ex: 18650), um \textit{Battery Management System} (BMS) \cite{plett2015}, e conversores \textit{Buck/Boost} para manter a estabilidade dos 5V/5A requeridos pela RPi e a linha de 12V dos discos rígidos.
...
\begin{itemize}  
    ...
    \begin{itemize}  
        ...
        \item Ferramentas de gerenciamento de disco (mdadm, LVM ou utilitários ext4/Btrfs).
    ...
    \begin{itemize}  
        ...
        \item Componentes Eletrônicos para o No-Break: Células Li-ion 18650, Placa BMS 3S/4S, Conversores DC-DC, capacitores, resistores e mosfets.
        ...
        \item Material PLA para \textit{case} externa.
    \end{itemize}  
\end{itemize}  
...
\begin{table}[H]
    \centering
    \caption{Matriz de Análise e Mitigação de Riscos do Projeto}
    \label{tab:analise_riscos}
    \renewcommand{\arraystretch}{1.4}
    
    \begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}X c c c >{\raggedright\arraybackslash}X c c @{}}
        \toprule
        ...
        Atraso na entrega de componentes importados (ex: \textit{Hat} PCIe, BMS) & 4 & 4 & \textbf{16*} &  Realizar a compra de componentes críticos com 1 mês de antecedência ou buscar fornecedor local. & 1 & 4 \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{} l >{\raggedright\arraybackslash}X c c c c >{\raggedright\arraybackslash}X l @{}}
        ...
        \multicolumn{8}{c}{\textbf{Marco 1: Sistema Base e Interface (29/04/2026)}} \\
        \midrule
        M1.1 & Aquisição e testes básicos de hardware & 15 & 01/04 & 10/04 & Marco & Componentes validados & - \\
        M1.2 & Compilação e configuração inicial do SO & 20 & 11/04 & 20/04 & Fernando & Imagem do SO pronta & M1.1 \\
        M1.3 & Interface Raspberry Pi x Usuário (SSH/Web) & 15 & 21/04 & 29/04 & Gabriel & Acesso remoto funcional & M1.2 \\
        
        \midrule
        \multicolumn{8}{c}{\textbf{Marco 2: Serviços e Energia (20/05/2026)}} \\
        \midrule
        M2.1 & Projeto e montagem do circuito No-Break & 25 & 30/04 & 10/05 & Gabriel & PCB/Circuito funcional & M1.1 \\
        ...
        \midrule
        \multicolumn{8}{c}{\textbf{Marco 3: Estrutura Física e Arrefecimento (03/06/2026)}} \\
        \midrule
        M3.1 & Montagem do circuito de dissipação de calor & 15 & 21/05 & 27/05 & Gabriel & Fans com controle PWM & M2.2 \\
        M3.2 & Modelagem e impressão do CASE 3D e Visor & 25 & 21/05 & 31/05 & Marco & Peças plásticas impressas & M1.1 \\
        M3.3 & Organizar sistema no CASE (pré-montagem) & 10 & 01/06 & 03/06 & Marco & Acomodação validada & M3.1, M3.2 \\
        
        \midrule
        \multicolumn{8}{c}{\textbf{Marco 4 e Conclusão: Integração e Validação (24/06/2026)}} \\
        \midrule
        ...
        M5.1 & Redação e revisão da documentação final & 15 & 18/06 & 24/06 & Todos & Documento finalizado & M4.2 \\
...
\begin{table}[H]
    ...
    \begin{tabular}{l c}
        ...
        \midrule
        Fernando Frare Vieira & 68.3 \\
        Gabriel Affonso Borges Caballero & 68.3 \\
        Marco Vieira Busetti & 63.4 \\
        \midrule
        \textbf{Total Estimado para o Projeto} & \textbf{200 horas} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{description}
    \item[\textit{Bit rot}] Degradação silenciosa e progressiva de dados armazenados causada por falhas físicas em células de memória, interferência eletromagnética ou erros não detectados de leitura/escrita, sem que o sistema sinalize o erro ao usuário.

    \item[Btrfs] \textit{B-Tree Filesystem}: sistema de arquivos avançado do Linux com suporte nativo a checksums de dados, snapshots e detecção de \textit{bit rot}, sendo a escolha preferencial quando a integridade dos dados é prioritária.

    \item[\textit{Cold storage}] Modalidade de armazenamento para dados raramente acessados, que prioriza baixo custo por GB em detrimento da velocidade de acesso.

    \item[Contêineres] Unidades de software que empacotam uma aplicação e todas as suas dependências em um ambiente isolado e reproduzível, compartilhando o \textit{kernel} do sistema operacional hospedeiro sem necessidade de uma máquina virtual completa.

    \item[Conversores \textit{Buck/Boost}] Circuitos eletrônicos de comutação que ajustam a tensão de entrada para um nível desejado: o conversor \textit{Buck} reduz a tensão (ex: 12V para 5V) e o \textit{Boost} a eleva (ex: 3,7V da bateria para 5V).

    \item[\textit{Daemons}] Processos executados em segundo plano pelo sistema operacional, sem interação direta com o usuário, responsáveis por gerenciar serviços contínuos como servidores de rede, compartilhamento de arquivos e monitoramento.

    \item[ext4] \textit{Fourth Extended Filesystem}: sistema de arquivos padrão do Linux, amplamente suportado e consolidado. Mais simples que o Btrfs, sem checksums nativos, mas robusto e com excelente compatibilidade com ferramentas de recuperação.

    \item[Fontes chaveadas] \textit{Switched-mode power supplies}: circuitos de alimentação que convertem tensão com alta eficiência energética usando transistores como chaves, ao contrário das fontes lineares que dissipam o excesso como calor.

    \item[Gentoo Linux] Distribuição Linux orientada à compilação do código-fonte, que permite otimizar cada pacote para o hardware específico. Resulta em um sistema extremamente enxuto, sem pacotes desnecessários, ideal para dispositivos embarcados de recursos limitados.

    \item[\textit{Kernel}] Núcleo do sistema operacional, responsável por gerenciar diretamente o hardware (processador, memória, dispositivos de I/O) e prover uma interface controlada para os demais programas do sistema.

    \item[\textit{Overhead}] Carga computacional adicional imposta por um sistema além da tarefa principal que executa, como gerenciamento de memória, protocolos de comunicação e controle de processos.

    \item[\textit{Thermal throttling}] Mecanismo de proteção do processador que reduz automaticamente sua frequência de operação ao atingir a temperatura máxima permitida, degradando o desempenho para evitar danos permanentes ao \textit{hardware}.
\end{description}
